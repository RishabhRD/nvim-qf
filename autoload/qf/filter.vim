...
function! s:SetList(pat, range, reject, strategy)
    ...
    if exists("b:qf_isLoc")
        if b:qf_isLoc == 1
            ...
            execute get(g:, "qf_auto_resize", 1) ? 'lclose|' . min([ max_height, len(getloclist(0)) ]) . 'lopen' : 'lclose|lopen'
        else
            " bufname && text
            if a:strategy == 0
            ...
            execute get(g:, "qf_auto_resize", 1) ? 'cclose|' . min([ max_height, len(getqflist()) ]) . 'copen' : 'cclose|copen'
        endif
    endif
endfunction
    ...
    if exists("b:qf_isLoc")
        if b:qf_isLoc == 1
            ...
            if len(lists) > 0
                ...
                execute get(g:, "qf_auto_resize", 1) ? 'lclose|' . min([ max_height, len(getloclist(0)) ]) . 'lopen' : 'lopen'
            ...
            if exists("g:qf_quickfix_lists")
                if len(g:qf_quickfix_lists) > 0
                    ...
                    execute get(g:, "qf_auto_resize", 1) ? 'cclose|' . min([ max_height, len(getqflist()) ]) . 'copen' : 'copen'
